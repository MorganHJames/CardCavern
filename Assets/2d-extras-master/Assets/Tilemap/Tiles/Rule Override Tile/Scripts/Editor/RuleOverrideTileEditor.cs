    ...
    internal class RuleOverrideTileEditor : Editor
    {
        ...
        public void DrawOriginalRuleElement(Rect rect, RuleTile.TilingRule originalRule, bool isDefault = false)
        {
            using (new EditorGUI.DisabledScope(true))
            {
                ...
                ruleTileEditor.RuleMatrixOnGUI(overrideTile.m_Tile, matrixRect, sibMatrixRect,originalRule);
        ...
        private void RuleOverrideInspectorOnGUI(Rect rect, RuleTile.TilingRule originalRule)
        {
			...
			overrideRule.m_CheckSiblings = EditorGUI.Toggle(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), overrideRule.m_CheckSiblings);
			y += k_SingleLineHeight;

			if (overrideRule.m_CheckSiblings)
			{
				GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Sibling Tile");
				overrideRule.m_SiblingTile = EditorGUI.ObjectField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), overrideRule.m_SiblingTile, typeof(TileBase), false) as TileBase;
				y += k_SingleLineHeight;
			}
		}
        ...
        private float GetRuleElementHeight(RuleTile.TilingRule rule)
        {
            ...
            if (rule != null)
            {
				...
				if (rule.m_CheckSiblings)
					height +=  k_SingleLineHeight + 4 + k_PaddingBetweenRules;
            }
            return height;
        }
    }

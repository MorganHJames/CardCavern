...
namespace UnityEditor
{
    ...
    internal class RuleTileEditor : Editor
    {
        ...
        internal const float k_DefaultElementHeight = 60f;
            ...
            if (tile.m_TilingRules != null && tile.m_TilingRules.Count > 0)
            {
                switch (tile.m_TilingRules[index].m_Output)
                {
					case RuleTile.TilingRule.OutputSprite.Single:
						if (tile.m_TilingRules[index].m_CheckSiblings)
							return k_DefaultElementHeight + k_SingleLineHeight + 4 + k_PaddingBetweenRules;
						break;

					case RuleTile.TilingRule.OutputSprite.Random:
						if (tile.m_TilingRules[index].m_CheckSiblings)
							return k_DefaultElementHeight + k_SingleLineHeight * (tile.m_TilingRules[index].m_Sprites.Length + 3) + k_PaddingBetweenRules + k_SingleLineHeight + 4 + k_PaddingBetweenRules;
						else
					...
					case RuleTile.TilingRule.OutputSprite.Animation:
						if (tile.m_TilingRules[index].m_CheckSiblings)
							return k_DefaultElementHeight + k_SingleLineHeight * (tile.m_TilingRules[index].m_Sprites.Length + 2) + k_PaddingBetweenRules + k_SingleLineHeight +4 + k_PaddingBetweenRules;
						else
        ...
        private void OnDrawElement(Rect rect, int index, bool isactive, bool isfocused)
        {
			...
			Rect sibMatrixRect = new Rect(rect.xMax - matrixWidth * 2f - 10f, yPos + 80f, matrixWidth, k_DefaultElementHeight);
			Rect spriteRect = new Rect(rect.xMax - matrixWidth - 5f, yPos, matrixWidth, k_DefaultElementHeight);

            EditorGUI.BeginChangeCheck();
            RuleInspectorOnGUI(inspectorRect, rule);
            RuleMatrixOnGUI(tile, matrixRect, sibMatrixRect, rule);
        ...
        private void OnAddElement(ReorderableList list)
        {
			...
			rule.m_CheckSiblings = tile.m_DefaultCheckSiblings;
        ...
        internal void RuleNeighborUpdate(Rect rect, Rect sibRect, RuleTile.TilingRule tilingRule, int index)
        {
			...
			if (Event.current.type == EventType.MouseDown && ContainsMousePosition(sibRect))
			{
				var allConsts = tile.m_SiblingNeighborType.GetFields(BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy);
				var neighbors = allConsts.Select(c => (int)c.GetValue(null)).ToList();
				neighbors.Sort();

				int oldIndex = neighbors.IndexOf(tilingRule.m_SiblingNeighbors[index]);
				int newIndex = (int)Mathf.Repeat(oldIndex + GetMouseChange(), neighbors.Count);
				tilingRule.m_SiblingNeighbors[index] = neighbors[newIndex];
		...
		internal void RuleSiblingNeighborUpdate(Rect rect, Rect sibRect, RuleTile.TilingRule tilingRule, int index)
		{
			if (Event.current.type == EventType.MouseDown && ContainsMousePosition(rect))
			{
				var allConsts = tile.m_NeighborType.GetFields(BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy);
		...
		internal void RuleTransformUpdate(Rect rect, Rect sibRect, RuleTile.TilingRule tilingRule)
        {
            if (Event.current.type == EventType.MouseDown && ContainsMousePosition(rect))
            {
        ...
        internal static void RuleInspectorOnGUI(Rect rect, RuleTile.TilingRule tilingRule)
        {
			...
			GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Check Siblings");
			tilingRule.m_CheckSiblings = EditorGUI.Toggle(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_CheckSiblings);
			y += k_SingleLineHeight;

			if (tilingRule.m_CheckSiblings)
			{
				GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Sibling Tile");
				tilingRule.m_SiblingTile = EditorGUI.ObjectField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_SiblingTile, typeof(TileBase), false) as TileBase;
				y += k_SingleLineHeight;
			}
		}
